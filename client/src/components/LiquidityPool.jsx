  ...
  const addLiquidity = async () => {
    ...
    try {
      ...
      if (reserveA.gt(0) && reserveB.gt(0)) {
        const totalReserves = reserveA.add(reserveB);
        const k = amount.div(totalReserves);
        amountA = k.mul(reserveA);
        amountB = k.mul(reserveB);
        
        // Ensure we don't send zero amounts
        if (amountA.isZero() || amountB.isZero()) {
          throw new Error("Liquidity amount too small for current reserves");
        }
      } else {
        amountA = amount;
        amountB = amount;
